        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case SensorService.MSG_SEND_SENSOR_DATA: {
                	...
                	if(_graphView != null) {
                		_graphView.drawSensorData(sensorData);
                	}
                	break;
                }
                case SensorService.MSG_SEND_SPIKE_DATA: {
                	...
                	String logstr = String.format("[%d] [%d, %.3f] (%d:%.3f, %d:%.3f, %d:%.3f)", 
            				...
            				spikeInfo.maxTime, spikeInfo.maxValue,
            				spikeInfo.endTime, spikeInfo.endValue);
                	Logger.log(logstr);
                	if(_toggleButton.isChecked()) {
                		outputText(2, logstr);
                	}
                ...
                case SensorService.MSG_SET_COUNT: {
                	...
                	if(_graphView != null) {
                		_graphView.drawSensorData(new SensorData("", 0, 0));
                	}
                	break;
                }
                default: {
